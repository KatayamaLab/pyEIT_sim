...
print("=== Experiment B: Statistical Position Accuracy Evaluation (GREIT) ===")
...
for noise_level in noise_levels_B:
    ...
    for day in days_list:
        ...
        if not np.isnan(mean_pe):
            print(
                f"Day {day:2d} ({contrast:4.2f}x): "
                f"PE = {mean_pe:.4f} ± {std_pe:.4f} (mean ± std, n={len(position_errors_trials)})"
            )
        else:
            print(
...
for idx, noise_level in enumerate(noise_levels_B):
    ...
    # 空のデータを除外
    non_empty_trials = [trials for trials in all_trials if len(trials) > 0]
    non_empty_contrasts = [
        c for c, trials in zip(contrasts_list, all_trials) if len(trials) > 0
    ]
...
for noise_level in noise_levels_B:
    ...
    if valid_indices:
        ...
        valid_days = [days_list[i] for i in valid_indices]
        valid_contrasts = [contrasts_list[i] for i in valid_indices]

        print(f"\n  Noise Level: {noise_level * 100:.1f}%")
        print("    - Best case (lowest mean PE):")
        min_idx_in_valid = np.argmin(valid_mean_errors)
        print(
            f"      Day {valid_days[min_idx_in_valid]} ({valid_contrasts[min_idx_in_valid]:.2f}x): "
            f"{valid_mean_errors[min_idx_in_valid]:.4f} ± {valid_std_errors[min_idx_in_valid]:.4f}"
        )

        print("    - Worst case (highest mean PE):")
        max_idx_in_valid = np.argmax(valid_mean_errors)
        print(
            f"      Day {valid_days[max_idx_in_valid]} ({valid_contrasts[max_idx_in_valid]:.2f}x): "
            f"{valid_mean_errors[max_idx_in_valid]:.4f} ± {valid_std_errors[max_idx_in_valid]:.4f}"
        )
